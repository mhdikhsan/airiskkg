...
w("---")
w("")
w("## Risk pattern → taxonomy: explicit triples, not inferred from names")
w("")
w("Risk pattern names read like OWASP entries (`PromptInjection`,")
w("`SystemPromptLeakage`), which invites the assumption that the taxonomy link is")
w("recovered from the name. It is not. There is no `mapsToRisk` predicate and no")
w("string matching anywhere in the pipeline. Every risk pattern carries two")
w("explicit sets of triples to resolvable URIs:")
w("")
w("- **`pair:derivedFrom`** — the single entry the pattern was written from.")
w("- **`pair:mayIndicateRisk`** — every taxonomy entry a finding may cite. Named")
w("  for Rule R1: a match *may indicate* a risk, it does not *map to* one.")
w("")
w("Name-based inference would in fact get several of these wrong:")
w("")
w("| | |")
w("|---|---|")
w("| `DirectPromptingWithoutGrounding` **and** `MisinformationFromWeakGrounding` | both anchor to `llm09-misinformation` — two patterns, one OWASP entry |")
w("| `ToolMisuse` | derives from **ASI**02, yet also indicates `llm06-excessive-agency` |")
w("| `MemoryPoisoning` | derives from **ASI**06, yet also indicates `llm04-data-and-model-poisoning` |")
w("| `VectorAndEmbeddingWeakness` | indicates `ibm-risk-atlas:prompt-injection`, which its name never suggests |")
w("")
w("Two tests hold the chain together:")
w("")
w("- `test_risk_patterns_have_condition_mechanism_and_taxonomy_anchor` — a risk")
w("  pattern without an anchor fails.")
w("- `test_may_indicate_risk_entries_are_mapped_to_anchor` — every *non-anchor*")
w("  entry must be joined to the anchor by a real SKOS mapping triple in the")
w("  taxonomy layer. Atlas and MIT entries cannot float free; they are reachable")
w("  only through a mapping that itself now carries a semapv justification in")
w("  `ontology/taxonomy/provenance/`.")
w("")
w("So the full evidence chain behind one suggested control is:")
w("")
w("```")
w("  motif --hasMotif--> risk pattern --derivedFrom--> OWASP entry")
w("                            |")
w("                            +--mayIndicateRisk--> Atlas / MIT / ASI entries")
w("                            |                     (each SKOS-mapped to the anchor,")
w("                            |                      each mapping justified)")
w("                            +--suggestedControl--> control")
w("```")
w("")
w("Only the last hop is uncorroborated project curation. Everything to its left")
w("is either a published pattern catalogue or a taxonomy link with recorded")
w("provenance.")
w("")
w("| Risk pattern | Anchor (`derivedFrom`) | Also indicates (`mayIndicateRisk`) |")
w("|---|---|---|")
for rp in rps:
    df = sorted(g.objects(rp, P.derivedFrom), key=str)
    anchor = n(df[0]) if df else "—"
    others = sorted(
        (n(x) for x in g.objects(rp, P.mayIndicateRisk) if not df or x != df[0]),
        key=str,
    )
    w(
        f"| `{n(rp).replace('RiskPattern','')}` | `{anchor}` | "
        + (", ".join(f"`{o}`" for o in others) or "—")
        + " |"
    )
w("")
w("---")
